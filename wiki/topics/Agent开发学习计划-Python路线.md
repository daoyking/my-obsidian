...
> 为 Swift 原生 macOS 开发者定制的 12 周 Agent 开发路线：LLM API 零基础、目标=个人自动化工具、每周约 5 小时（共约 60h）、Python 为主路线。与既有 [[Agent开发学习计划]]（前端求职 6 周 TS 路线）互补：一个是求职向，一个是个人工具向。求职向可配合 [[Agent开发工程师面试宝典]]（题目按本周次映射，W0-W12）。
